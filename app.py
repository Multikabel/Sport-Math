...
# --- VÝBĚR TÝMŮ PRO SIMULACI ---
    ...
    # --- ROHY (xC – Expected Corners Model) ---
    ...
    # 4) Dominance index (shots + dribbles + xG)
    ...
    league_dom_avg = 13.0  # PL průměr dominance

    h_rohy_pro *= 1 + (dom1 - league_dom_avg) * 0.03
    a_rohy_pro *= 1 + (dom2 - league_dom_avg) * 0.03

    # 5) Field tilt (útočná převaha)
    # kolik % útočných akcí jde jedním směrem
    tilt = (dom1 + 1e-6) / (dom1 + dom2 + 1e-6)

    h_rohy_pro *= 0.85 + tilt * 0.30
    a_rohy_pro *= 1.15 - tilt * 0.30

    # 6) Tempo zápasu (součet střel)
    tempo = ws1["shots"] + ws2["shots"]
    tempo_factor = 1 + (tempo - 24) * 0.015

    h_rohy_pro *= tempo_factor
    a_rohy_pro *= tempo_factor

    # 7) Rohy povolené soupeřem (defenzivní styl)
    # kolik rohů soupeř běžně dovolí
    ...
    h_rohy_pro *= 1 + (allowed_t2 - 5.0) * 0.05
    a_rohy_pro *= 1 + (allowed_t1 - 5.0) * 0.05

    # 8) Výsledné expected corners (xC)
